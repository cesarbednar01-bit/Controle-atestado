...
const COLLECTION_NAME = "atestados";
...
function daysBetweenInclusive(start, end) {
  ...
  return Math.round(
    (dateLocal(end) - dateLocal(start)) / 86400000
  ) + 1;
}
...
function getOffDaysCount(c) {
  if (Array.isArray(c.offDays)) {
    return c.offDays.length;
  }

  if (c.offStart && c.offEnd) {
    return daysBetweenInclusive(c.offStart, c.offEnd);
  }
...
function createdAtMillis(c) {
  ...
  if (typeof c.createdAt.toMillis === "function") {
    return c.createdAt.toMillis();
  }

  if (typeof c.createdAt.seconds === "number") {
...
function startCertificatesListener() {
  if (unsubscribeCertificates) {
  ...
  unsubscribeCertificates = onSnapshot(
    certificatesRef,
    snapshot => {
      certificates = snapshot.docs.map(documentSnapshot => ({
...
async function saveCertificate(data) {
  if (!currentUser) {
    showToast("Sua sessão não está autenticada.");
    return false;
  }
...
document.addEventListener("click", event => {
  ...
  if (targetButton) {
    event.preventDefault();
    navigate(targetButton.dataset.pageTarget);
    return;
  }

  const quickLaunch =
    event.target.closest("#quickLaunch");
...
function updateOffDaysVisibility() {
  const selected =
    document.querySelector(
      'input[name="teveFolga"]:checked'
    );

  const hasOffDays =
    selected?.value === "sim";

  $("periodoFolga")?.classList.toggle(
    "hidden",
    !hasOffDays
  );

  if (!hasOffDays) {
    if ($("folgaInicio")) $("folgaInicio").value = "";
    if ($("folgaFim")) $("folgaFim").value = "";
  }
...
function validateOffPeriod() {
  ...
  if (
    ...
    certificateEnd &&
    (
      offStart < certificateStart ||
      offStart > certificateEnd
    )
  ) {
    $("folgaInicio")?.setCustomValidity(
  ...
  if (
    ...
    certificateEnd &&
    (
      offEnd < certificateStart ||
      offEnd > certificateEnd
    )
  ) {
    $("folgaFim")?.setCustomValidity(
...
if ($("certificateForm")) {
  $("certificateForm").addEventListener(
    ...
    async event => {
      ...
      if (days < 1 || days > 30) {
        showToast(
          "O atestado deve ter entre 1 e 30 dias."
        );
      ...
      if (hasOffDays) {
        ...
        if (!offStart || !offEnd) {
          showToast(
            "Informe a data inicial e final da folga."
          );
...
function updateDashboard() {
  ...
  if ($("metricSickDays")) {
    $("metricSickDays").textContent =
      stats.sick;
  }

  if ($("metricOffDays")) {
    $("metricOffDays").textContent =
      stats.off;
  }
  ...
  $("recentTable").innerHTML =
    recent.length
      ? recent.map(c => `
        <tr>
          <td>${escapeHtml(getCertificateEmployee(c))}</td>
          <td>${escapeHtml(getCertificateChief(c))}</td>
...
function renderHistory() {
  ...
  const {
    filtered,
    invalidRange
  } = getHistoryFilteredCertificates();
...
function exportHistoryPDF() {
  ...
  if (!filtered.length) {
    alert(
      "Não existem atestados para os filtros selecionados."
    );
  ...
  doc.text(
    "Dias de atestado por chefe de turno",
    margin,
    y
  );
  ...
  filtered.forEach(
    (c, index) => {
      ...
      addWrapped(
        `Período do atestado: ${formatDate(c.start)} até ${formatDate(c.end)}`,
        margin + 3,
        pageWidth -
          margin * 2 -
          3,
      ...
      if (
        ...
        addWrapped(
          `Folga informada: ${formatDate(c.offStart)} até ${formatDate(c.offEnd)} (${offCount} dia(s))`,
          margin + 3,
          pageWidth -
            margin * 2 -
            3,
      ...
      addWrapped(
        `Dias considerados como atestado: ${affected} dia(s)`,
        margin + 3,
        pageWidth -
          margin * 2 -
          3,
  ...
  for (
    ...
    doc.text(
      "Controle de Atestados | Relatório operacional",
      margin,
      pageHeight - 9
    );
  ...
  doc.save(
    `relatorio-atestados-${safeStart}-${safeEnd}.pdf`
  );
}
...
if ($("applyDashboard")) {
  $("applyDashboard").addEventListener(
    "click",
    updateDashboard
  );
}
